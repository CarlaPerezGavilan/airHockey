...
class Player:
    def __init__(self, paddle_pos, goal_side):
        ...
        self.my_display_name = "test TEAM"
    ...
    def next_move(self, current_state):
        ...
        roi_radius = (current_state['board_shape'][0] * current_state['goal_size'])*self.offset
        pt_in_roi = None
        for p in path:
        ...
        if pt_in_roi:
            ...
            calculate_x = (((roi_radius**2)-((target['y']-self.my_goal_center['y'])**2))**(1/2))+self.my_goal_center['x']
            if(self.kick_off==True):
                calculate_x=target['x']
            if(self.kick_off==True and current_state['puck_pos']['x']>(current_state['board_shape'][1]/2) and self.my_goal=="left"):
                self.kick_off=False
                self.offset=0.7
            if(self.kick_off==True and current_state['puck_pos']['x']<(current_state['board_shape'][1]/2) and self.my_goal=="right"):
                self.kick_off=False
                self.offset=0.7
            target_pos = {'x': calculate_x, 'y':  target['y']}
